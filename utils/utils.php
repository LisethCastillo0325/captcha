...
class Utils{
    ...
    static function obtenerUrlCompletaVercaptcha($s, $use_forwarded_host=false ) {
        $urlScript = explode("index.php", $s['SCRIPT_NAME']);
        $uri = str_replace(constant('NOMBRE_PROYECTO'), constant('NOMBRE_PROYECTO_VERCAPTCHA'), $urlScript[0]);
        return self::obtenerUrlOrigen($s, $use_forwarded_host ).$uri;
    }
